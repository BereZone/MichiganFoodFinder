...
- **Wolverine Village** dining hall, now that it has menus published. Added to the Python scraper, the browser-relay extension, and the Browse hall filter. The extension is bumped to 1.1.0 and must be reloaded (`chrome://extensions` -> Reload) before it will pick the hall up.
