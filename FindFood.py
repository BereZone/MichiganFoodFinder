        ...
        if found_meal_tab:
            ...
            if menu_section:
                ...
                for item in menu_items:
                    ...
                    item_name = re.sub(r'\s+', ' ', item.get_text(strip=True))
                    menu_items_counter[item_name] += 1
    return menu_items_counter
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for menu_items_counter in results:
            total_counter.update(menu_items_counter)
        total_unique_items = len(total_counter)
        # Write menu items and counts to a text file
        with open('menu_items_with_counts.txt', 'w', encoding='utf-8') as file:
            file.write(f"Total Unique Menu Items: {total_unique_items}\n")
            file.write("Menu Items with Counts:\n")
            for item, count in total_counter.most_common():
                file.write(f"{item}: {count}\n")
        # Print the total number of unique items
        print(f"Total Unique Menu Items: {total_unique_items}")
